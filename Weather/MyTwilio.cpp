	...
	std::string& response)
{
	...
	char *body_encoded_special_char = curl_easy_escape(
		client.get_handle(), body.c_str(), 0);
	std::string parameter_string = "To=" + to + "&From=" + from + 
		"&Body=" + body_encoded_special_char;

	client.set_url(url_string);
	client.set_post_parameters(parameter_string);
...
int MyTwilio::display_details(bool flag)
{
